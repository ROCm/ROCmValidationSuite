#!/usr/bin/env bash
# Build (and tag) the ROCm runtime docker image for RVS nightly docker tests.
...
#       (requires *-Linux.tar.gz with -rMMmm.yyyymmdd-; nightly pins exact 10.1.0a20260819 SDK
#        from therock-dist-linux-multiarch-<version>.tar.gz)
...
# Match SDK to RVS *-Linux.tar.gz release tag (rMMmm.yyyymmdd), same encoding as
# build_packages_local.sh CPACK release (r<ROCM_LIBPATCH_VERSION>.<yyyymmdd>).
...
resolve_sdk_base "$ROCM_VERSION"
IMAGE_TAG="${IMAGE_TAG:-rvs-nightly-rocm:${ROCM_VERSION}}"
...
docker tag "${IMAGE_TAG}" "rvs-nightly-rocm:latest"
echo "::notice::Tagged ${IMAGE_TAG} and rvs-nightly-rocm:latest"
