...
- Downloads existing `release/unsigned/rpm/` RPM files from S3, excluding `repodata/` (which will be fully regenerated)
- Downloads matching `.rpm` files from `release/rvs/rpm/`
- Runs `createrepo_c` (fallback: `createrepo`) to regenerate `repodata/`
- Syncs the staging directory back to S3
...
```json
{
  ...
  "rpm": {
    ...
    "s3_key": "release/unsigned/rpm/amdrocm7-rvs-1.3.15-r0711.20260423.x86_64.rpm"
  },
  "tar": {
...
| RPM repodata | `createrepo_c` | `createrepo_c` |
...
# RPM: verify package and repodata
aws s3 ls "s3://${BUCKET}/release/unsigned/rpm/" | grep "${BUILD_NUM}"
aws s3 ls "s3://${BUCKET}/release/unsigned/rpm/repodata/"
...
baseurl=https://<bucket>.s3.amazonaws.com/release/unsigned/rpm/
enabled=1
gpgcheck=0
EOF
